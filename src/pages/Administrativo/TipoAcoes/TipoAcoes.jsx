...
export default function TipoAcoes(acoes_tipos) {

    const navigate = useNavigate()
    console.log("ACOES", acoes_tipos?.acoes_tipos);



    const colunas = [
        { head: 'Tipo Ações', value: 'descricao' },
    ]

    const handleAdd = () => navigate('/manutencao/cadastro_acoes-tipo')

    const handleEdit = (id) => {
        console.log(id);
        navigate(`/manutencao/cadastro_acoes-tipo/${id}`)
    }
    ...
    return (
        <div>
            ...
            <div class="m-2 md:m-10 p-2 bg-white  rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
                ...
                <TabelaSimples dados={acoes_tipos?.acoes_tipos} colunas={colunas} edit handleEdit={handleEdit} />
            </div>
        </div >
