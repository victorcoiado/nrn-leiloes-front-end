...
export default function Cartorios(cartorios_data) {
    const [cartorios, setCartorios] = useState(cartorios_data?.cartorios_data)
    console.log(cartorios_data);
    ...
    const colunas = [
        ...
        { head: 'Razão Social', value: 'razaoSocial' },
        { head: 'Endereço', value: 'endereco' },
    ]

    const handleAdd = () => navigate('/manutencao/cadastro_cartorio')

    const handleEdit = (obj) => {

        navigate(`/manutencao/cadastro_cartorio/${obj.id}`)
    }
    ...
    const handleDelete = async (obj) => {
        try {
            ...
            var res = await axios.get(
                `${getApiLeadUrl()}/nrn/cartorios/del?id=${obj.id}`,
                {
                    headers: headers,
                }
            );
    ...
    return (
        <div>
            <div className="mt-2 ml-2 sm:mt-20 md:mt-20 md:ml-10 bg-white rounded-3xl dark:bg-main-dark-bg">
                <Header title="Cartórios" category="Aqui você encontra os cartórios cadastrados" />
            ...
            <div class="m-2 md:m-10 p-2 bg-white  rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
                    ...
                    <div class="flex justify-between my-4 relative">
                        ...
                        <div>
                            <button
                                type="submit"
                                onClick={handleAdd}
                                className="mr-0 sm:mr-4 text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"><BsBuildingFillAdd /></button>
                        </div>
                    </div>
                ...
                <TabelaSimples dados={cartorios} colunas={colunas} edit handleEdit={handleEdit} handleDelete={handleOpenModal} />
            </div>
